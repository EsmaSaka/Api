...
public class ORDSTestsDay5 {
     ...
     */
    @Test
    @DisplayName(" user verifies that average salary is grater that 5000")
    public void test1(){
       Response response= given().accept(ContentType.JSON)
                .when().get("/employees");
      assertEquals(200,response.getStatusCode());
      List<Integer> salary=response.jsonPath().getList("items.salary");
      double avgSalary= salary.stream().mapToDouble(p->p).average().getAsDouble();
        assertTrue(avgSalary>5000,"ERROR: actually average salary is lower than 5000:"+avgSalary);
